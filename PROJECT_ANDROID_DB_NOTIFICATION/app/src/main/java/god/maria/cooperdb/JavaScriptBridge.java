...
public class JavaScriptBridge {
    ...
    public JavaScriptBridge(Context c, NotificationManager notificationManager, DatabaseHelper myDb, EventCache eventCache, ServerConnexion serverConnexion, DatabaseAndroidActivity databaseAndroidActivity) {
        this.mContext = c;
    ...
    @JavascriptInterface
    public String getEventFromServerOrAndroidDb(String service, String connexion) throws JSONException{
        String result = null;
        if (connexion.equals("on")) {
            result = serverConnexion.resultFromServer(service);
            databaseAndroidActivity.insertDataInAndroidIfNew(result);
        }
        if (connexion.equals("off")){
