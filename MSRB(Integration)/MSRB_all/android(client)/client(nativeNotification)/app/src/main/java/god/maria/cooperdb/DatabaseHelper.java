...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // String to declare the elements in the database and table
    ...
    // Creating Table in android database
    @Override
    ...
    public Cursor eventQuery(){
        Cursor result = this.getReadableDatabase().rawQuery("SELECT * FROM " + TABLE_NAME + " ORDER BY " + COL_ID + " DESC " + " LIMIT 10", null);
        return result;
    }

    // Getting the last 10 barks included in the android database according to a rawquery
    public Cursor barkQuery(){
        Cursor result = this.getReadableDatabase().rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE "
                + COL_EVENT + " = ? " + " ORDER BY " + COL_ID + " DESC " + " LIMIT 10", new String[]{"bark"});
        return result;
    }

    // Getting the last 10 movements included in the android database according to a rawquery
    public Cursor movQuery(){
        Cursor result = this.getReadableDatabase().rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE "
                + COL_EVENT + " = ? " + " ORDER BY " + COL_ID + " DESC " + " LIMIT 10", new String[]{"movement"});
        return result;
    }

    // Inserting data in android database
